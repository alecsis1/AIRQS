...
public enum Command {
	 ...
	 */
	GET_HUMIDITY("print(sensehat.get_humidity())"), GET_TEMPERATURE("print(sensehat.get_temperature())"), GET_TEMPERATURE_FROM_HUMIDITY("print(sensehat.get_temperature_from_humidity())"), GET_TEMPERATURE_FROM_PRESSURE("print(sensehat.get_temperature_from_pressure())"), GET_PRESSURE("print(sensehat.get_pressure())"),
	 ...
	 * LED Matrix
	 */
	SET_ROTATION("sensehat.set_rotation(%s)"), SET_PIXEL("sensehat.set_pixel(%s, %s, %s, %s, %s)"), SET_PIXELS("sensehat.set_pixels([%s])"), CLEAR("sensehat.clear()"), CLEAR_WITH_COLOR("sensehat.clear(%s, %s, %s)"), SHOW_MESSAGE("sensehat.show_message('%s')"), SHOW_MESSAGE_PARAMETRIZED("sensehat.show_message('%s', %s, [%s, %s, %s], [%s, %s, %s])"), SHOW_LETTER("sensehat.show_letter('%s')"), SHOW_LETTER_PARAMETRIZED("sensehat.show_letter('%s', [%s, %s, %s], [%s, %s, %s])"), LOW_LIGHT("sensehat.low_light = %s"),

	/**
	 * Joystick
	 */
	GET_EVENTS("e = sensehat.stick.get_events();" + "print(str('|'.join('{}@{}@{}'.format(i.action, i.direction, i.timestamp) for i in e)))"), WAIT_FOR_EVENT_EMPTY_BUFFER("e = sensehat.stick.wait_for_event(%s)\n" + "print('{}@{}@{}'.format(e.action, e.direction, e.timestamp))"),;
